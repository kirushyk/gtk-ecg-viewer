...
void tlc5000_bin_cache(TLC5000Bin *priv, int first_frame, int last_frame)
{
    ...
    for (frame = first_frame; frame <= last_frame; frame++)
    {
        ...
        int chunk_number = frame / FRAMES_PER_CHUNK;
        int chunk_position = frame - chunk_number * FRAMES_PER_CHUNK;
        fseek(priv->file, chunk_number * CHUNK_SIZE + 8 + chunk_position * FRAME_SIZE, SEEK_SET);
        fread(&buffer, 1, FRAME_SIZE, priv->file);
...
int tlc5000_bin_read(TLC5000Bin *priv, int channel, int frame)
{
    ...
    int chunk_number = frame / FRAMES_PER_CHUNK;
    int chunk_position = frame - chunk_number * FRAMES_PER_CHUNK;
    long int position = chunk_number * CHUNK_SIZE + 8 + chunk_position * FRAME_SIZE + (channel / 2 * 3);
